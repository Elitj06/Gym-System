...
const menuItems = [
  ...
  { href: '/biometria', label: 'Biometria', icon: ScanFace },
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed top-3 left-3 z-50 md:hidden bg-gym-surface border border-gym-border rounded-lg p-2 text-gym-text-secondary hover:text-white transition-colors"
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-screen bg-gym-surface border-r border-gym-border z-50 transition-all duration-300 flex flex-col
          ${collapsed ? 'w-[72px]' : 'w-[240px]'}
          ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0
        `}
      >
        {/* Logo */}
        <div className="h-16 flex items-center justify-between px-4 border-b border-gym-border">
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="md:hidden text-gym-text-muted hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-3 border-t border-gym-border hidden md:block">
          <button

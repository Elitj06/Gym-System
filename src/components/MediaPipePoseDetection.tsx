...
export default function MediaPipePoseDetection({ onResults, onScoreUpdate, videoUrl, isLive = false, facingMode = 'environment' }: Props) {
  ...
  // Keep refs updated without re-renders
  ...
  // Stable pose result handler
  const handlePoseResult = useCallback((r: any) => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (cancelled) return
        drawRef.current = dm
        connRef.current = pm.POSE_CONNECTIONS
        const pose = new pm.Pose({
  ...
  // Camera - uses facingMode directly, re-runs when it changes
  useEffect(() => {
    ...
    const startCamera = async () => {
      try {
        ...
        // Try to enumerate devices to find rear camera
        ...
        if (facingMode === 'environment' && videoDevices.length > 1) {
          ...
          try {
            ...
            // Try selecting the last device (usually rear camera on mobile)
            try {
        ...
        setCameraLabel(settings.facingMode === 'environment' ? '📷 Traseira' : settings.facingMode === 'user' ? '🤳 Frontal' : track.label?.substring(0, 20) || '')
        ...
        // Use requestAnimationFrame loop instead of @mediapipe/camera_utils
        // This is more reliable for camera switching
        ...
        if (!cancelled) {
          if (e.name === 'NotAllowedError') setError('Permissão de câmera negada. Permita o acesso nas configurações.')
          else if (e.name === 'NotFoundError') setError('Nenhuma câmera encontrada.')
          else setError(`Erro ao acessar câmera: ${e.message}`)
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (!v) return
    let interval: any = null, cancelled = false
    const proc = async () => {
      if (poseRef.current && v.readyState >= 2 && !v.paused) {
  ...
  if (error) {
    return (
      <div className="bg-red-500/10 border border-red-500/50 rounded-lg p-4 text-red-200">
        ...
        <button onClick={() => { setError(null); setLoading(true) }} className="mt-3 px-4 py-2 bg-red-500/20 rounded-lg text-sm flex items-center gap-2">
  ...
  return (
    <div className="relative w-full h-full">
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-gym-dark/80 z-10 rounded-lg">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-gym-accent mx-auto mb-4" />
            <p className="text-gym-text-secondary text-sm">Carregando IA...</p>
          </div>
        </div>
      )}
      ...
      {detected && (
        <div className="absolute top-2 left-2 bg-green-500/90 text-white px-2 py-1 rounded text-xs font-medium flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse" /> Pose Detectada
        </div>
      )}
      {cameraLabel && isLive && (
